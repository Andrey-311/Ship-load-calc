...
def get_ecr_list(project_id: int, status_filter: str = None, db: Session = Depends(get_db)):
    """Получить список заявок проекта."""
    uow = UnitOfWork(db)
    ecrs = uow.ecrs.get_by_project(project_id, status=status_filter)

    result = []
    for ecr in ecrs:
        lines = uow.lines.get_by_ecr(ecr.id)
...
def create_ecr(project_id: int, ecr_data: ECRCreate, db: Session = Depends(get_db)):
    ...
    result = uow.ecrs.create(ecr)
    uow.commit()
    return result


@router.get("/{ecr_id}", response_model=ECRResponse)
def get_ecr(project_id: int, ecr_id: int, db: Session = Depends(get_db)):
    """Получить заявку по ID."""
    uow = UnitOfWork(db)
    ecr = uow.ecrs.get_by_id(ecr_id)
...
def get_ecr_lines(project_id: int, ecr_id: int, db: Session = Depends(get_db)):
...
def update_load_line(
    ...
    line = uow.lines.get_by_id(line_id)
    ...
    update_data = {k: v for k, v in line_data.model_dump().items()
                   if v is not None}
    result = uow.lines.update(line_id, **update_data)
    uow.commit()
    return result


@router.delete("/lines/{line_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_load_line(project_id: int, ecr_id: int, line_id: int, db: Session = Depends(get_db)):
    ...
    uow.lines.delete(line_id)
    uow.commit()
    return None


@router.get("/{ecr_id}/center-of-gravity", response_model=CenterOfGravityResponse)
def get_ecr_center_of_gravity(project_id: int, ecr_id: int, db: Session = Depends(get_db)):
